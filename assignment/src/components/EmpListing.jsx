...
const EmpListing = () => {
  ...
  const handleDelete = (id) => { // added id as a parameter
    deleteEmployee(id)
     .then(() => {
            ...
            {employees && employees.map(item => (
              <tr key={item.id}>
                ...
                <td className="px-6 py-4 border border-slate-600">
                  ...
                  <Link  className="px-4 py-2 mr-2 font-bold text-white bg-red-500 rounded hover:bg-red-700" onClick={()=>{handleDelete(item.id)}} >Remove</Link>
